...
In the following example, we assume a simple BIDS-like project structure 
with subjects (here called ``ID000001``, ...) and two experiments/time points 
(``TP000``, ``TP001``) for each:
...
Batch processing using ZenReg's helpers
----------------------------------------

We demonstrate batch processing using the synthetic batch project 
created by ``write_batch_example_project`` and ZenReg's batch processing 
helper ``iter_bids_like_image_files``.
...
Discover all subjects whose folder name starts with ``ID``:
...
.. list-table::
   ...
   * - Argument
     - Meaning
   * - ``project_root``
     - Required root folder that contains subject folders.
   * - ``subject_ids``
     - Explicit subject folders to process. Use ``None`` to discover all
       matching subjects. Default:  ``None``.
   * - ``experiment_tags``
     - Experiment folders to include, for example ``("TP000", "TP001")``. ``None`` (default) discovers matching experiment folders.
   * - ``subject_prefix`` / ``experiment_prefix``
     - Folder-name prefixes used for automatic discovery. Defaults:  ``"ID"`` and ``"TP"``.
...
.. code-block:: python
   ...
   for record in records:
       ...
       save_stack(output_path, registered, 
                  metadata             = metadata, 
...
The helpers above are intentionally small. If your project structure differs, it
...
.. code-block:: python
   ...
   for subject_dir in subject_dirs:
       experiment_dirs = sorted(path for path in subject_dir.iterdir()
                                if path.is_dir()
                                and path.name.startswith("TP")
                                and path.name in selected_experiments)

       for experiment_dir in experiment_dirs:
           image_files = sorted(path for path in experiment_dir.iterdir()
                                if path.is_file() 
                                and path.name.lower().endswith(image_extensions))
